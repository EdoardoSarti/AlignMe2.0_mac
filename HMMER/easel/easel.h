...
/* Most of this support is in the alphabet module, but we extern "C"alize 
...
extern "C" void esl_exception(int errcode, int use_errno, char *sourcefile, int sourceline, char *format, ...);
extern "C" void esl_exception_SetHandler(esl_exception_handler_f);
extern "C" void esl_exception_ResetDefaultHandler(void);
extern "C" void esl_nonfatal_handler(int errcode, int use_errno, char *sourcefile, int sourceline, char *format, va_list argp);
extern "C" void esl_fatal(const char *format, ...);

/* 2. Memory allocation/deallocation conventions. */
extern "C" void esl_Free2D(void  **p, int dim1);
extern "C" void esl_Free3D(void ***p, int dim1, int dim2);

/* 3. Standard banner for Easel miniapplications. */
extern "C" int  esl_banner(FILE *fp, char *progname, char *banner);
extern "C" int  esl_usage (FILE *fp, char *progname, char *usage);

/* 4. Improved replacements for some C library functions */
extern "C" int  esl_fgets(char **buf, int *n, FILE *fp);
extern "C" int  esl_strdup(const char *s, int64_t n, char **ret_dup);
extern "C" int  esl_strcat(char **dest, int64_t ldest, const char *src, int64_t lsrc);
extern "C" int  esl_strmapcat        (const ESL_DSQ *inmap, char **dest, int64_t *ldest, const char *src, esl_pos_t lsrc);
extern "C" int  esl_strmapcat_noalloc(const ESL_DSQ *inmap,  char *dest, int64_t *ldest, const char *src, esl_pos_t lsrc);
extern "C" int  esl_strtok    (char **s, char *delim, char **ret_tok);
extern "C" int  esl_strtok_adv(char **s, char *delim, char **ret_tok, int *opt_toklen, char *opt_endchar);
extern "C" int  esl_sprintf (char **ret_s, const char *format, ...);
extern "C" int  esl_vsprintf(char **ret_s, const char *format, va_list *ap);
extern "C" int  esl_strcmp(const char *s1, const char *s2);
...
#else
extern "C" int  esl_strcasecmp(const char *s1, const char *s2);
#define strcasecmp esl_strcasecmp
#endif
#endif

/* 6. Additional string functions, esl_str*() */
extern "C" int     esl_strchop(char *s, int64_t n);
extern "C" int     esl_strdealign(char *s, const char *aseq, const char *gapchars, int64_t *opt_rlen);
extern "C" int     esl_str_IsBlank(char *s);
extern "C" int     esl_str_IsInteger(char *s);
extern "C" int     esl_str_IsReal(char *s);
extern "C" int64_t esl_str_GetMaxWidth(char **s, int n);

/* 7. File path/name manipulation functions, including tmpfiles */
extern "C" int  esl_FileExists(const char *filename);
extern "C" int  esl_FileTail(const char *path, int nosuffix, char **ret_file);
extern "C" int  esl_file_Extension(char *filename, esl_pos_t n_ignore, char **ret_sfx, esl_pos_t *ret_n);
extern "C" int  esl_FileConcat(const char *dir, const char *file, char **ret_path);
extern "C" int  esl_FileNewSuffix(const char *filename, const char *sfx, char **ret_newpath);
extern "C" int  esl_FileEnvOpen(const char *fname, const char *env,
			    FILE **ret_fp, char **ret_path);
extern "C" int  esl_tmpfile(char *basename6X, FILE **ret_fp);
extern "C" int  esl_tmpfile_named(char *basename6X, FILE **ret_fp);
extern "C" int  esl_getcwd(char **ret_cwd);

/* 8. Typed comparison routines. */
extern "C" int  esl_DCompare   (double a, double b, double tol);
extern "C" int  esl_FCompare   (float  a, float  b, float  tol);
extern "C" int  esl_DCompareAbs(double a, double b, double tol);
extern "C" int  esl_FCompareAbs(float  a, float  b, float  tol);
extern "C" int  esl_CCompare(char *s1, char *s2);

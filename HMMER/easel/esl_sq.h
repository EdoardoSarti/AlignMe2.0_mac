...
extern "C" ESL_SQ *esl_sq_Create(void);
extern "C" ESL_SQ *esl_sq_CreateFrom(const char *name, const char *seq,
				 const char *desc, const char *acc, const char *ss);
extern "C" int     esl_sq_Grow  (ESL_SQ *sq, int64_t *ret_nsafe);
extern "C" int     esl_sq_GrowTo(ESL_SQ *sq, int64_t  n);
extern "C" int     esl_sq_Copy(const ESL_SQ *src, ESL_SQ *dst);
extern "C" int     esl_sq_Compare  (ESL_SQ *sq1, ESL_SQ *sq2);
extern "C" int     esl_sq_Reuse    (ESL_SQ *sq);
extern "C" int     esl_sq_IsDigital(const ESL_SQ *sq);
extern "C" int     esl_sq_IsText   (const ESL_SQ *sq);
extern "C" void    esl_sq_Destroy  (ESL_SQ *sq);

extern "C" int     esl_sq_SetName        (ESL_SQ *sq, const char *name);
extern "C" int     esl_sq_SetAccession   (ESL_SQ *sq, const char *acc);
extern "C" int     esl_sq_SetDesc        (ESL_SQ *sq, const char *desc);
extern "C" int     esl_sq_SetSource      (ESL_SQ *sq, const char *source);
extern "C" int     esl_sq_FormatName     (ESL_SQ *sq, const char *name,   ...);
extern "C" int     esl_sq_FormatAccession(ESL_SQ *sq, const char *acc,    ...);
extern "C" int     esl_sq_FormatDesc     (ESL_SQ *sq, const char *desc,   ...);
extern "C" int     esl_sq_FormatSource   (ESL_SQ *sq, const char *source, ...);
extern "C" int     esl_sq_AppendDesc     (ESL_SQ *sq, const char *desc);
extern "C" int     esl_sq_SetCoordComplete(ESL_SQ *sq, int64_t L);
extern "C" int     esl_sq_CAddResidue (ESL_SQ *sq, char c);
extern "C" int     esl_sq_ReverseComplement(ESL_SQ *sq);
extern "C" int     esl_sq_Checksum(const ESL_SQ *sq, uint32_t *ret_checksum);
extern "C" int     esl_sq_CountResidues(const ESL_SQ *sq, int start, int L, float *f);

#ifdef eslAUGMENT_ALPHABET
extern "C" ESL_SQ *esl_sq_CreateDigital(const ESL_ALPHABET *abc);
extern "C" ESL_SQ *esl_sq_CreateDigitalFrom(const ESL_ALPHABET *abc, const char *name, const ESL_DSQ *dsq, 
					int64_t L, const char *desc, const char *acc,  const char *ss);
extern "C" int     esl_sq_Digitize(const ESL_ALPHABET *abc, ESL_SQ *sq);
extern "C" int     esl_sq_Textize(ESL_SQ *sq);
extern "C" int     esl_sq_GuessAlphabet(ESL_SQ *sq, int *ret_type);
extern "C" int     esl_sq_XAddResidue(ESL_SQ *sq, ESL_DSQ x);
extern "C" int     esl_sq_ConvertDegen2X(ESL_SQ *sq);
#endif

#ifdef eslAUGMENT_MSA
extern "C" int     esl_sq_GetFromMSA  (const ESL_MSA *msa, int which, ESL_SQ *sq);
extern "C" int     esl_sq_FetchFromMSA(const ESL_MSA *msa, int which, ESL_SQ **ret_sq);
#endif

extern "C" ESL_SQ_BLOCK *esl_sq_CreateBlock(int count);
#ifdef eslAUGMENT_ALPHABET
extern "C" ESL_SQ_BLOCK *esl_sq_CreateDigitalBlock(int count, const ESL_ALPHABET *abc);
#endif
extern "C" void          esl_sq_DestroyBlock(ESL_SQ_BLOCK *sqBlock);

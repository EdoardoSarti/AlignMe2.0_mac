...
extern "C" int  esl_sqfile_Open(const char *seqfile, int fmt, const char *env, ESL_SQFILE **ret_sqfp);
extern "C" int  esl_sqfile_Position(ESL_SQFILE *sqfp, off_t offset);
extern "C" void esl_sqfile_Close(ESL_SQFILE *sqfp);

#ifdef eslAUGMENT_ALPHABET
extern "C" int  esl_sqfile_OpenDigital(const ESL_ALPHABET *abc, const char *filename, int format, const char *env, ESL_SQFILE **ret_sqfp);
extern "C" int  esl_sqfile_SetDigital(ESL_SQFILE *sqfp, const ESL_ALPHABET *abc);
extern "C" int  esl_sqfile_GuessAlphabet(ESL_SQFILE *sqfp, int *ret_type);

extern "C" int  esl_sqfile_Cache(const ESL_ALPHABET *abc, const char *seqfile, int fmt, const char *env, ESL_SQCACHE **ret_sqcache);
extern "C" void esl_sqfile_Free(ESL_SQCACHE *sqcache);
#endif

const char  *esl_sqfile_GetErrorBuf(const ESL_SQFILE *sqfp);
extern "C" int   esl_sqfile_IsRewindable(const ESL_SQFILE *sqfp);
extern "C" int   esl_sqio_Ignore(ESL_SQFILE *sqfp, const char *ignoredchars);
extern "C" int   esl_sqio_AcceptAs(ESL_SQFILE *sqfp, char *xchars, char readas);
extern "C" int   esl_sqio_EncodeFormat(char *fmtstring);
extern "C" char *esl_sqio_DecodeFormat(int fmt);
extern "C" int   esl_sqio_IsAlignment(int fmt);

extern "C" int   esl_sqio_Read        (ESL_SQFILE *sqfp, ESL_SQ *sq);
extern "C" int   esl_sqio_ReadInfo    (ESL_SQFILE *sqfp, ESL_SQ *sq);
extern "C" int   esl_sqio_ReadWindow  (ESL_SQFILE *sqfp, int C, int W, ESL_SQ *sq);
extern "C" int   esl_sqio_ReadSequence(ESL_SQFILE *sqfp, ESL_SQ *sq);
extern "C" int   esl_sqio_Echo        (ESL_SQFILE *sqfp, const ESL_SQ *sq, FILE *ofp);
extern "C" int   esl_sqio_ReadBlock   (ESL_SQFILE *sqfp, ESL_SQ_BLOCK *sqBlock, int max_residues, int max_sequences, int long_target);

#ifdef eslAUGMENT_SSI
extern "C" int   esl_sqfile_OpenSSI         (ESL_SQFILE *sqfp, const char *ssifile_hint);
extern "C" int   esl_sqfile_PositionByKey   (ESL_SQFILE *sqfp, const char *key);
extern "C" int   esl_sqfile_PositionByNumber(ESL_SQFILE *sqfp, int which);

extern "C" int   esl_sqio_Fetch      (ESL_SQFILE *sqfp, const char *key, ESL_SQ *sq);
extern "C" int   esl_sqio_FetchInfo  (ESL_SQFILE *sqfp, const char *key, ESL_SQ *sq);
extern "C" int   esl_sqio_FetchSubseq(ESL_SQFILE *sqfp, const char *source, int64_t start, int64_t end, ESL_SQ *sq);
#endif

extern "C" int   esl_sqio_Write(FILE *fp, ESL_SQ *s, int format, int update);
extern "C" int   esl_sqio_Parse(char *buffer, int size, ESL_SQ *s, int format);

...
/* The visible API.
 */
extern "C" ESL_GETOPTS *esl_getopts_Create(ESL_OPTIONS *opt);
extern "C" ESL_GETOPTS *esl_getopts_CreateDefaultApp(ESL_OPTIONS *options, int nargs, int argc, char **argv, char *banner, char *usage);
extern "C" int          esl_getopts_Reuse  (ESL_GETOPTS *g);
extern "C" void         esl_getopts_Destroy(ESL_GETOPTS *g);
extern "C" void         esl_getopts_Dump(FILE *ofp, ESL_GETOPTS *g);

extern "C" int esl_opt_ProcessConfigfile (ESL_GETOPTS *g, char *filename, FILE *fp);
extern "C" int esl_opt_ProcessEnvironment(ESL_GETOPTS *g);
extern "C" int esl_opt_ProcessCmdline    (ESL_GETOPTS *g, int argc, char **argv);
extern "C" int esl_opt_ProcessSpoof      (ESL_GETOPTS *g, const char *cmdline);
extern "C" int esl_opt_VerifyConfig      (ESL_GETOPTS *g);
extern "C" int esl_opt_ArgNumber   (const ESL_GETOPTS *g);
extern "C" int esl_opt_SpoofCmdline(const ESL_GETOPTS *g, char **ret_cmdline);

extern "C" int esl_opt_GetSetter(const ESL_GETOPTS *g, char *optname);

extern "C" int    esl_opt_IsDefault (const ESL_GETOPTS *g, char *optname);
extern "C" int    esl_opt_IsOn      (const ESL_GETOPTS *g, char *optname);
extern "C" int    esl_opt_IsUsed    (const ESL_GETOPTS *g, char *optname);

extern "C" int    esl_opt_GetBoolean(const ESL_GETOPTS *g, char *optname);
extern "C" int    esl_opt_GetInteger(const ESL_GETOPTS *g, char *optname);
extern "C" double esl_opt_GetReal   (const ESL_GETOPTS *g, char *optname);
extern "C" char   esl_opt_GetChar   (const ESL_GETOPTS *g, char *optname);
extern "C" char  *esl_opt_GetString (const ESL_GETOPTS *g, char *optname);
extern "C" char  *esl_opt_GetArg    (const ESL_GETOPTS *g, int which);

extern "C" int esl_opt_DisplayHelp(FILE *ofp, ESL_GETOPTS *go, int docgroup, int indent, int textwidth);

...
void ReadSequencesProfilesAndScalesForAlignPairs( AlignmentVariables &VARS)
{
	  ...
	  if( VARS.first_msa_file == "" && VARS.second_msa_file == "")
	  {
		  ...
		  ReadSimilarityScoreFile( VARS.scores, VARS.HMMProfilePair, VARS.second_fasta_file, VARS.first_sequence, VARS.second_sequence, VARS.similarity_score_file, VARS.defined_amino_acids, VARS.write_profiles_header1, VARS.write_profiles_header2, VARS.first_fasta_id, VARS.second_fasta_id);
		  DebugWrite( "first sequence: " << VARS.first_sequence << "\nsecond sequence: " << VARS.second_sequence);
	  }

// EDO: ATTENZIONE, MSA NON MODIFICATO!

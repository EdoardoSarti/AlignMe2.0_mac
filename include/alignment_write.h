...
void WriteAlignmentIndices( const std::pair< double, std::vector< std::pair< int, int> > > &ALIGNMENT, const std::string &TOFILE, const size_t &LINE_LENGTH)
{
  std::ofstream write( TOFILE.c_str());
...
//	std::cout << __FUNCTION__ << " " << GAP_EXTENSION_PENALTY << "  " << FILE << std::endl;
  ...
  if( !write)
  {
	  std::cout << "WARNING: It was not possible to open the file (" << TOFILE << ") to which the profiles should be written to! \n";
  }

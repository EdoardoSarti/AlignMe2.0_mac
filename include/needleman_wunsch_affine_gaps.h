...
private:
	...
	ShPtr< Function< std::pair< GeneralizedAminoAcid, GeneralizedAminoAcid>, double> >   m_Score;
        std::pair< ShPtr< Vec3>, ShPtr< Vec3> >   m_HMMProfilePair;
        ShPtr< Vec3> m_HMMProfile;
	ShPtr< Vec3> m_HMMForward;
	std::vector< double> m_uncertainty;
	Matrix< DynamicProgrammingMatrixElement>                       m_Matrix;
	std::pair< int, int>				m_EndCoord;
	DynamicProgrammingMatrixElement			m_EndElement;
...
public:
	...
	void CalculateUncertainty(std::vector< double> &m_uncertainty);
    
	void CalculateMatrix();

        void CalculateMatrix_HMM();

...
/*
 *  1) The itr goes through the amino acid sequence that has been read in (e.g. first_sequence or second_sequence).
 *  2) itr->GetType() returns the m_Type (which is in this case an AA) that is stored at the position to which itr points to.
 *     This value will be searched in Scale_Map in which values corresponding to a certain AA are stored as pairs.
 *  3) With ->second you have access to this corresponding value.
 *     This value will then be added to the sequence that the position that itr points to.
 *  4) Because of AddNewProfile(value) the value will be pushed back in m_Profiles of the corresponding GAA.
 */
...
void ReadScaleIntoSequence(Sequence &SEQUENCE,const std::map<char, double> &SCALE_MAP)
{
    ...
    {
        //  std::map<>::find() returns an iterator to key/value std::pair.
        //  It has to be used rather then operator [] because map is passed as const.
...
)
{
    ...
    //  Reads the scale_file and assign to every amino_acid (first column) the corresponding
    //  hydrophobicity-value (second column).
    //  Stores a pair (AA-Type, Hydrophocity) in scale.
    while( read)
    {
#ifdef SECURE
        aa_str.clear();
...
HiddenMarkovModel ReadHiddenMarkovModelMatrix( const std::string &HMM_FILE)
{
...
// after this, there are the values for the amino acids
	...
	if( !read)
	{
		std::cerr << "ERROR: The HPSSM file [" << HMM_FILE << "] was not found" << "\n";
		exit( -1);
	}
	...
	if (splitter.size() < 21 )
	{
		std::cerr << "ERROR: A list of all amino acids was not found in the HPSSM file [" << HMM_FILE << "] \n";
		exit( -1);
	}
#endif
	...
	splitter = mystr::SplitString(line, " ");
        ...
        if (splitter.size() < 7 )
        {
                std::cerr << "ERROR: A list of all state transition probabilities was not found in the HPSSM file [" << HMM_FILE << "] \n";
                exit( -1);
        }
#endif

	//EDO: COMPO is not mandatory and contains the average substitution probabilities
	std::getline (read, line);
        splitter = mystr::SplitString(line, " ");
	if (splitter[0] == "COMPO") 
        {
            ...
            {
                value = mystr::ConvertStringToNumericalValue< double>(splitter[it+1]);
                HMM.SetSubs(0, HMM.GetAA(it), value);
            }
        }
        //EDO: Insertion, Node zero

        std::getline (read, line);
        splitter = mystr::SplitString(line, " ");
        for(int it=0; it<20; ++it)
            {
                value = mystr::ConvertStringToNumericalValue< double>(splitter[it]);
                HMM.SetIns(0, HMM.GetAA(it), value);
            }

        //EDO: Transition probabilities from Node zero
	std::getline (read, line);
        splitter = mystr::SplitString(line, " ");
        for(int it=0; it<7; ++it)
            {
                value = mystr::ConvertStringToNumericalValue< double>(splitter[it]);
                HMM.SetState(0, HMM.GetStateName(it), value);
            }
	...
	while (read)
	{
	    ...
	    if (splitter.size() < 21)
	    {
		std::cerr << "The line < "<< line << " > in your HPSSM is not formatted correctly. There have to be at least 21 columns in a HMM file\n";
		std::cerr << "Please check the HPSSM file [" << HMM_FILE << "] for a correct formatting \n";
		exit( -1);
	    }

	    for (size_t hmm_pos = 1; hmm_pos < 21; ++hmm_pos)
	    {
#ifdef SECURE
		if( !mystr::IsNumerical(splitter[hmm_pos]))
		{
		    std::cerr << "ERROR: The value: <" << value << "> is not numerical although it is expected to be \n";
		    std::cerr << "Please check the HPSSM file [" << HMM_FILE << "] for a correct formatting \n";
	            exit( -1);
...
#endif
		value = mystr::ConvertStringToNumericalValue< double> (splitter[hmm_pos]);

		HMM.SetSubs(HMM.GetLastNode(), HMM.GetAA(hmm_pos-1), value);
	    }
            ...
            if (splitter.size() < 20)
            {
                std::cerr << "The line < "<< line << " > in your HPSSM is not formatted correctly. There have to be at least 20 columns in a HMM file\n";
                std::cerr << "Please check the HPSSM file [" << HMM_FILE << "] for a correct formatting \n";
                exit( -1);
            }

            for (size_t hmm_pos = 0; hmm_pos < 20; ++hmm_pos)
            {
#ifdef SECURE
                if( !mystr::IsNumerical(splitter[hmm_pos]))
                {
                ...
                HMM.SetIns(HMM.GetLastNode(), HMM.GetAA(hmm_pos), value);
            }
            ...
            if (splitter.size() < 7)
            {
                std::cerr << "The line < "<< line << " > in your HPSSM is not formatted correctly. There have to be at least 7 columns in a HMM file\n";
                std::cerr << "Please check the HPSSM file [" << HMM_FILE << "] for a correct formatting \n";
                exit( -1);
            }

            for (size_t hmm_pos = 0; hmm_pos < 7; ++hmm_pos)
            {
#ifdef SECURE
                if( !(mystr::IsNumerical(splitter[hmm_pos])) && (splitter[hmm_pos] != "*"))
                {
                    std::cerr << "ERROR: The value: <" << value << "> is not numerical although it is expected to be \n";
                    std::cerr << "Please check the HPSSM file [" << HMM_FILE << "] for a correct formatting \n";
                    exit( -1);
                ...
                HMM.SetState(HMM.GetLastNode(), HMM.GetStateName(hmm_pos), value);
            }
	}
	read.close();
	read.clear();

	HMM.SetEmissions(); // It compiles a vector of maps which goes with the same order of the transition matrix

	std::cout << __FUNCTION__ << " LAST NODE: " << HMM.GetLastNode() << std::endl;
        return HMM;
}


std::pair<ShPtr< Vec3>, ShPtr< Vec3> > CreateHMMProfile( const std::string &HMMfile, const std::string &second_fasta_file, Sequence &SECOND)
{
        HiddenMarkovModel HMM = ReadHiddenMarkovModelMatrix( HMMfile);
	std::cout << __FUNCTION__ << std::endl;
	char *HMMfilename = (char *) malloc(sizeof(char) * strlen(HMMfile.c_str()) + 1);
	char *fastafilename = (char *) malloc(sizeof(char) * strlen(second_fasta_file.c_str()) + 1);

	std::strcpy(HMMfilename, HMMfile.c_str());
	std::strcpy(fastafilename, second_fasta_file.c_str());

	// Number of states: HMM->LastNode+1 (there is also the zeroth state, which has transitions and ins. emissions but not subs. emissions)
	// Number of amino acids: Sequence->size(), but columns in NW are Sequence->size() + 1
        ShPtr< Vec3> HMMProfile( new Vec3(SECOND.size()+1,HMM.GetLastNode()+1));
	std::cout << __FUNCTION__ << std::endl;
//	HMM.ConvertHMM(HMMProfile);
        HMM.HMMER_ConvertHMM(HMMfilename, fastafilename, HMMProfile);

//        return ForwardBackward(HMM, HMMProfile, SECOND);
	return std::make_pair(HMMProfile, HMMProfile);
}
...
//              ShPtr< Vec3> &HMMProfile,
//		ShPtr< Vec3> &ForwardHMMProfile,
		std::pair<ShPtr< Vec3>, ShPtr< Vec3> > &HMMProfilePair,
                std::string &second_fasta_file,
		Sequence &FIRST,
		Sequence &SECOND,
		  ...
		  else if ( type == "NormalizedProfileDependentSubstitutionMatrix")
		  {
			 ...
			 double profile_id = mystr::ConvertStringToNumericalValue<int>( splitter[9]);
			  ...
			  else
			  {
                  ...
                  else if( type == "HiddenMarkovModelProfile" )
                  {
#ifdef SECURE
                          if (splitter_size != 6) //weight, type, HMMfile, sequence
                          {
                                  std::cerr << "ERROR: In the similarity_score_file  [" << SCORE_AND_PROFILE_FILE << "], a line about " << type <<" does not fulfill the correct formatting rules. The format for  " << type <<" has to be: weight: <double> type: "<< type <<" file: <filename> \n";
                                  exit( -1);
                          }
...
#endif                    
                          HMMfile = splitter[5];
                          test_read.open( HMMfile.c_str());
#ifdef SECURE
                          if( !test_read)
                          {
                                  std::cerr << "ERROR: In the similarity_score_file  [" << SCORE_AND_PROFILE_FILE << "], the HMM profile [" <<   HMMfile << "] was not found";
                                  exit( -1);
                          }
                          test_read.close();
#endif
                          HMMProfilePair = CreateHMMProfile(HMMfile, second_fasta_file, SECOND);
                          ...
                          {
                                  SCORES = ShPtr< Function< std::pair< GeneralizedAminoAcid, GeneralizedAminoAcid>, double> >( new ScoreNullProfile());
                                  SCORES = 0.0 * SCORES;
                          }
...
/*
		  else if( type == "HMMPositionSpecificSimilarity" )
		  {
#ifdef SECURE
			  if (splitter_size != 8)
			  {
			  ...
			  if( type == "HMMPositionSpecificSimilarity")
			  {
				  if( SCORES)
				  {
					  SCORES =  SCORES + weight * ShPtr< Function< std::pair< GeneralizedAminoAcid, GeneralizedAminoAcid>, double> >( new ScoreHMMPositionSpecificSimilarity());
				  }
				  else
				  {
					  SCORES = ShPtr< Function< std::pair< GeneralizedAminoAcid, GeneralizedAminoAcid>, double> >( new ScoreHMMPositionSpecificSimilarity());
					  SCORES = weight * SCORES;
				  }
			  }
		  }
